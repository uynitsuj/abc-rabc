...
def _put_model_and_data_safe(
    ...
    *,
    ...
    def _create_array_with_safe_sizes(data, spec, sizes, *args, **kwargs):
        # mujoco_warp 3.10 added a trailing batch_size arg; pass extras through
        # so this shim works on both 3.9 and 3.10.
...
class WarpReplayRuntime:
    ...
    def forward_kinematics(self) -> None:
        """Position stage only (kinematics + com + camera/lights) for rendering.

        Skips the collision/constraint solve: geom and camera world poses come
        purely from kinematics, so the rendered image is identical to a full
        forward(), but without the expensive collision narrowphase whose EPA
        buffers can OOM on a GPU shared with other jobs. Mirrors the GL replay
        path's mj_kinematics / mj_comPos / mj_camlight.
        """
        self._mjw.kinematics(self.m_warp, self.d_warp)
        self._mjw.com_pos(self.m_warp, self.d_warp)
        self._mjw.camlight(self.m_warp, self.d_warp)
