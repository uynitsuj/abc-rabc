...
def _run_one_batch(
    ...
    *,
    ...
    for _chunk_idx in range(max_chunks):
        ...
        if not active:
            break
        # One timelapse frame per executed chunk, for every recorded world --
        # not just `active`, so finished worlds keep a constant frame count and
        # the per-world clips stay index-aligned with record_worlds.
