...
class SimEvalConfig:
    ...
    # Local DiT checkpoint path. Optional: not needed when policy_backend="pi0",
    ...
    # When True (default, back-compat) an episode halts the instant all bottles
    # are placed-ever ("ever_success"). Set False to run the full num_chunks
    # horizon every episode — needed for offline re-scoring under a different
    # in-bin primitive, since early-stop truncates the qpos trace at the moment
    # the CURRENT primitive first reads 6/6 (and truncates the faster arm more).
    early_stop: bool = True
    save_video: bool = False
    video_fps: int = 30
    video_every_n_actions: int = 1
    prompt: str = "sim put the plastic bottles in the bin"
    # Prompt sent to the openpi pi0 server (policy_backend="pi0" only).
    # None keeps the historical hardcoded training prompt (PI0_PROMPT in
    # eval_policy.py), so existing eval commands are byte-identical. Set to
    # override per-run, e.g. for velocity-conditioned (WARP-VC) checkpoints:
    #   --pi0-prompt "Put the plastic bottles in the bin, speed: fast"
    pi0_prompt: str | None = None
