...
PY=.venv/bin/python
run() { local t=$1 ep=$2 gpu=$3
  $PY scripts/rerender_episode.py --episode-s3 $BASE/$t/$ep \
    --cache-dir "$CACHE" --out-dir "$OUT" --backend mjwarp --gpu-id $gpu --batch-size 64 \
    > "$LOG/$t.log" 2>&1 ; echo "[done exit=$?] $t"
}
